...
def backup_management():
    ...
    # Get all forms except those from the current month
    forms = Form.query.filter(
        Form.form_id.isnot(None),
        Form.form_id != '',
        Form.date_created < current_month_start
    ).all()
    
    # Group forms by month using date_created
    months_data = defaultdict(list)
    
    for form in forms:
        year = form.date_created.year
        month = form.date_created.month
        months_data[(year, month)].append(form)
    
    # Format data for template
    available_months = []
    for (year, month), month_forms in sorted(months_data.items(), reverse=True):
        month_name = calendar.month_name[month]
        available_months.append({
            'year': year,
            'month': month,
            'name': f'{month_name} {year}',
            'count': len(month_forms)
        })
    
    return render_template('backup_management.html', available_months=available_months)
...
def download_backup():
    ...
    try:
        # Get all forms for the specified month using date_created
        ...
        forms = Form.query.filter(
            Form.form_id.isnot(None),
            Form.form_id != '',
            Form.date_created >= month_start,
            Form.date_created < next_month_start
        ).all()
        ...
        for form in forms:
            ...
            uploads_src = os.path.join('uploads', form.form_id)
            if os.path.exists(uploads_src):
                uploads_dest = os.path.join(backup_dir, 'uploads', form.form_id)
        ...
        for form in forms:
            # Delete uploads
            uploads_dir = os.path.join('uploads', form.form_id)
...
def restore_backup():
    ...
    if not backup_file.filename.endswith('.zip'):
        flash('Invalid backup file format. Must be a ZIP file.', 'error')
        return redirect(url_for('backup_management'))
    
    # Create a temporary directory for restoration
    restore_dir = f'temp_restore_{datetime.now().strftime("%Y%m%d%H%M%S")}'
    os.makedirs(restore_dir, exist_ok=True)
    
    try:
        # Save and extract the backup file
        backup_path = os.path.join(restore_dir, 'backup.zip')
        ...
        db_backup_dir = os.path.join(restore_dir, 'database')
        if not os.path.exists(db_backup_dir):
            raise ValueError('Invalid backup file: missing database directory')
        
        restored_count = 0
        skipped_count = 0
        
        # Iterate through each form backup
        for json_file in os.listdir(db_backup_dir):
            if not json_file.endswith('.json'):
                continue
                
            with open(os.path.join(db_backup_dir, json_file)) as f:
                form_data = json.load(f)
            
            # Check if form already exists
            existing_form = Form.query.filter_by(form_id=form_data['form_id']).first()
            if existing_form:
                skipped_count += 1
                continue
            ...
            src_uploads = os.path.join(restore_dir, 'uploads', form_data['form_id'])
            if os.path.exists(src_uploads):
                dst_uploads = os.path.join('uploads', form_data['form_id'])
        ...
        flash(f'Backup restored successfully. {restored_count} forms restored, {skipped_count} forms skipped.', 'success')
        return redirect(url_for('backup_management'))
...
def create_form():
    ...
    if forms_count >= 50:
        flash("You have reached the maximum of 50 forms. Please clean up old forms with backup.", "error")
        return redirect(url_for('index'))
    ...
    if last_form:
        # Extract the number after the date prefix and increment
        current_num = int(last_form.form_id[8:]) # Skip the date part (YYYYMMDD)
        new_form_id = f'{date_prefix}{str(current_num + 1).zfill(3)}'
    else:
        # First form of the day
        new_form_id = f'{date_prefix}001'
...
def view_form(form_id):
    ...
    if os.path.exists(image_folder):
        uploaded_images = [
            ...
            and file != 'cover_image'  # Exclude cover_image directory
        ]
...
def duplicate_form(form_id):
    try:
        ...
        # Generate new form ID using current date and sequence number
        current_date = datetime.now()
        date_prefix = current_date.strftime("%Y%m%d")
        
        # Find the last form ID for today
        today_forms = Form.query.filter(
            Form.form_id.like(f"{date_prefix}%")
        ).order_by(Form.form_id.desc()).first()
        
        if today_forms:
            # Extract sequence number and increment
            seq_num = int(today_forms.form_id[-3:]) + 1
        else:
            # Start with 001 if no forms exist for today
            seq_num = 1
